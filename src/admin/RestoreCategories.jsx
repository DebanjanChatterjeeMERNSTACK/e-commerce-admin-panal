...
const RestoreCategories = () => {
  ...
  const categoryitem = () => {
    fetch(`${URL}/catagory_get`, {
      ...
      .then((json) => {
        if (json.status == 200) {
          setData(json.catagory.filter((e) => e.product_Catagory_Delete === 1));
        } else {
          Swal.fire({
            title: json.text,
  ...
  const handleRestore = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${URL}/catagory_restore`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ id: id, product_Catagory_delete_update: 0 }),
        })
          .then((res) => res.json())
          .then((data) => {
            if (data.status == 200) {
